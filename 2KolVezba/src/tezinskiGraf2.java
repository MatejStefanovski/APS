...
public class tezinskiGraf2 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < m; i++) {
            String task = sc.next();
            String dep = sc.next();
            graph.addEdge(dep, task, duration.get(task));
        ...
        for (String t : graph.getVertices()) {
            if (inDegree.get(t) == 0) {
                q.add(t);
                finishTime.put(t, duration.get(t));
            }
        }
        ...
        while (!q.isEmpty()) {
            ...
            for (Map.Entry<String, Integer> neighbor : graph.getNeighbors(u).entrySet()) {
                ...
                finishTime.put(v, Math.max(finishTime.getOrDefault(v, 0), finishTime.get(u) + w));
                inDegree.put(v, inDegree.get(v) - 1);
                if (inDegree.get(v) == 0) q.add(v);
            }
        }

        System.out.println(Collections.max(finishTime.values()));
    }
}
