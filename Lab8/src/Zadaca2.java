...
public class Zadaca2 {
    static int maxSuma = 0;
    static int soberi(BNode<Integer> node, int d){
        if (node == null){
            return 0;
        }
        int sum=0;
        ...
        int lsum=soberi(node.left,d+1);
        int dsum=soberi(node.right,d+1);
        return sum+lsum+dsum;
    }
    static void funkcija(BNode<Integer> node, int d){
        if (node == null){
            return;
        }
        maxSuma = Math.max(maxSuma, soberi(node, d));
        funkcija(node.left, d+1);
        funkcija(node.right, d+1);
    }
    public static void main(String[] args) {
        ...
        map.put(root, tree.root);
        for (int i=0; i<n-1; i++){
            int parent = sc.nextInt();
            int child = sc.nextInt();
            String strana = sc.next();
            BNode<Integer> p = map.get(parent);
            BNode<Integer> c = new BNode<>(child, p);
            map.put(child, c);
            if (strana.equals("LEFT")){
                p.left = c;
            }
            else{
                p.right = c;
