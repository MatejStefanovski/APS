...
public class Main {
    public static int brlistovi(SLLTree<String>.SLLNode<String> node) {
        if (node == null){
            return 0;
        }
    ...
    public static void main(String[] args) {
        ...
        Map<String, SLLTree<String>.SLLNode<String>> map = new HashMap<>();
        String root = sc.nextLine();
        tree.makeRoot(root);
        map.put(root, (SLLTree<String>.SLLNode<String>) tree.root());
        for (int i = 0; i < n - 1; i++) {
            String parent = sc.next();
            String child = sc.next();
            sc.nextLine();
            SLLTree<String>.SLLNode<String> parentNode = map.get(parent);
            SLLTree<String>.SLLNode<String> childNode = (SLLTree<String>.SLLNode<String>) tree.addChild(parentNode, child);
            map.put(child, childNode);
        }
        int m = Integer.parseInt(sc.nextLine());
        for (int i = 0; i < m; i++) {
            String nodeName = sc.nextLine();
            SLLTree<String>.SLLNode<String> node = map.get(nodeName);
            System.out.println(brlistovi(node));
        }
    }
}
